...
} from '../../src/broker/ticket-approval.js';
...
test('lists every exact execution item in a review-and-confirm follow-up', () => {
  ...
  assert.match(formatExecutionItemsMessage(tickets), /close N/);
  assert.equal(formatExecutionItemsMessage([{ number: 1, action: 'buy', symbol: 'XYZ' }]), '');
});

test('pending tickets have a factual fallback and trusted conversational context', () => {
  const ticketSet = { id: 'set-1', createdAt, tickets: extractReportTickets(report).tickets };
  assert.match(formatTicketOptions(ticketSet), /nothing has executed/i);
  assert.match(formatTicketOptions(ticketSet), /close all/i);
  const context = pendingTicketContext(ticketSet, { mode: 'semi' });
  assert.match(context, /^\[BRIDGE CONTEXT/);
  assert.match(context, /proposed-pending/);
  assert.match(context, /active mode approval gate/i);
});
...
test('dismisses selected or all pending tickets without broker action', () => {
  const ticketSet = { id: 'set-1', createdAt, tickets: extractReportTickets(report).tickets };
  const one = resolveTicketDismissal('close ticket 1', ticketSet);
  assert.equal(one.kind, 'dismissed');
  assert.deepEqual(one.numbers, [1]);
  assert.equal(one.all, false);

  const all = resolveTicketDismissal('never mind, close all tickets', ticketSet);
  assert.deepEqual(all.numbers, [1, 2]);
  assert.equal(all.all, true);
  assert.equal(resolveTicketDismissal('cancel SOFI at $17', ticketSet), null);
});

test('an unmatched explicit dismissal selector never clears the whole set', () => {
  const ticketSet = { id: 'set-1', createdAt, tickets: extractReportTickets(report).tickets };
  const result = resolveTicketDismissal('close ticket 9', ticketSet);
  assert.deepEqual(result.numbers, []);
  assert.deepEqual(result.requested, [9]);
...
test('approval prompt is exact and recovery requires live reconciliation', () => {
  ...
  assert.match(prompt, /at most 0\.3%/);
  assert.match(prompt, /fresh approval/);
