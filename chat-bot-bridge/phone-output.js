// Deterministic last-mile policy for text leaving the phone bridge.
...
function redactPrivateData(text) {
  ...
  out = out.split('\n').map((line) => {
    ...
    if (!privateValue) return line;
    return line.replace(MONEY_RE, () => { redactions++; return '[private account value]'; });
  }).join('\n');
