...
import dns from 'dns';
// Bridge application composition root. Root server.js remains a stable compatibility entrypoint.
import fs from 'fs';
import path from 'path';
import { spawn, spawnSync } from 'child_process';
import { fileURLToPath } from 'url';
import { chatRulesBase, modeRules, currentMode, setMode, MODES } from '../control/chat-rules.js';
import {
...
import { forbiddenSendPath, sanitizePhoneText } from '../delivery/phone-output.js';
import {
...
import {
  ...
  SCHEDULE_KINDS,
  scheduledLabel,
} from '../runtime/scheduled-task.js';
import { pacificLabel } from '../runtime/clock.js';
import {
  createRunTranscript,
  ...
  toolEventLine,
} from '../delivery/run-telemetry.js';
import {
  createRemoteRunView,
  finishRemoteRunView,
  formatPhoneHelp,
  ...
  parseRemoteControl,
  updateRemoteRunView,
} from '../control/remote-control.js';
import {
...
} from '../agents/availability-recovery.js';
import {
...
import {
  ...
  normalizedImageType,
  saveInboundImage,
} from '../delivery/inbound-media.js';
import { createBrokerPreflightTracker, scheduledBrokerCorrectionPrompt } from '../broker/broker-preflight.js';
import {
  buildChangeReview,
  captureWorkspaceSnapshot,
  parseChangeReason,
} from '../delivery/change-review.js';
import {
  ...
  resolveTicketDismissal,
} from '../broker/ticket-approval.js';
import {
  captureReportSnapshot,
  deliverReportArtifacts,
  generatedHtmlReports,
} from '../reports/report-delivery.js';
import {
  captureReportDraftSnapshot,
  recoverableReportDrafts,
} from '../reports/report-artifact-recovery.js';
import {
...
} from '../reports/report-recovery.js';
...
async function applyManualAgentChoice(replyTo, choice) {
  ...
  const fallback = currentMode() === 'full'
    ? `Validate-only shadow availability fallback remains automatic; other agent: ${alternatives.join(' → ') || 'none available'}`
...
async function handleInbound(replyTo, text, sentAtMs = null, attachments = [], continuationSnapshotDir = null, runOptions = {}) {
  ...
  if (cmd === '/mode' || cmd.startsWith('/mode ')) {
    ...
    return sendText(replyTo, `Mode set: ${want.toUpperCase()}\n${MODES[want]}\n\nApplies from the next message and the next scheduled run.${want === 'full' ? '\n\nAvailability failures may switch across configured models. Full remains validate-only: it never places a broker order. Kill switch: /mode manual' : ''}`);
  }
  ...
  if (brokerExecution) {
    ...
    if (approvableTicketSet?.tickets?.length) {
      const dismissal = resolveTicketDismissal(text, approvableTicketSet);
      if (dismissal?.kind === 'dismissed') {
        const droppedNumbers = new Set(dismissal.numbers);
        const remaining = dismissal.all
          ? []
          : approvableTicketSet.tickets.filter((ticket) => !droppedNumbers.has(ticket.number));
        const dropped = approvableTicketSet.tickets.filter(
          (ticket) => dismissal.all || droppedNumbers.has(ticket.number),
        );
        if (!dropped.length) {
          logEvent('ticket_dismiss_miss', {
            ticket_set: approvableTicketSet.id,
            requested: dismissal.requested || [],
          });
          return sendText(
            replyTo,
            `No matching ticket to close. Open: ${approvableTicketSet.tickets.map((ticket) => `#${ticket.number} ${ticket.symbol}`).join(', ')}. Reply “close N” or “close all”.`,
          );
        }
        state.pendingTicketSet = remaining.length
          ? { ...approvableTicketSet, tickets: remaining }
          : {
            ...approvableTicketSet,
            tickets: [],
            status: 'dismissed',
        ...
        if (!remaining.length && state.pendingBrokerExecution?.ticketSetId === approvableTicketSet.id) {
          delete state.pendingBrokerExecution;
        }
        saveState(state);
        logEvent('ticket_dismissed', {
          ticket_set: approvableTicketSet.id,
          dropped: dropped.map((ticket) => ticket.number),
          remaining: remaining.map((ticket) => ticket.number),
        });
        const label = dropped.map((ticket) => `${ticket.number}. ${ticket.description}`).join('\n');
        const tail = remaining.length
          ? `\nStill open: ${remaining.map((ticket) => `#${ticket.number}`).join(', ')} (reply “approve N” or “close N”).`
          : '\nNo protective order was touched. Nothing carries into the next report.';
        return sendText(replyTo, `🗑️ Closed — dropped from the queue:\n${label}${tail}`);
      }
    }
  ...
  if (!brokerExecution && !reportRequest && !answeredDecision && !answeredModelSwitch
      && state.pendingTicketSet?.status === 'pending') {
    const ticketContext = pendingTicketContext(state.pendingTicketSet, { mode: currentMode() });
    if (ticketContext) {
      agentPrompt = `${ticketContext}\n\nUser message: ${agentPrompt}`;
      logEvent('pending_ticket_context_injected', {
        ticket_set: state.pendingTicketSet.id,
        count: state.pendingTicketSet.tickets?.length || 0,
      });
    }
  }
  ...
  let loopRecoveryAttempted = false;
  ...
  if (!activeRun?.interrupt && shouldRecoverLoopedRun({
    result,
    prompt: agentPrompt,
    options: runOptions,
    brokerExecution: Boolean(brokerExecution),
    alreadyRecovered: loopRecoveryAttempted,
  })) {
    ...
    result = await runPreferredAgent(
      recoveryPrompt,
      onRunEvent,
      { agent: initialFailure.agent, model: initialFailure.model },
      runAttachments,
      {
        ...runOptions,
        autoModelFallback: false,
  ...
  if (ok) {
    ...
    } else if (reportRequest && !brokerExecution
        && state.pendingTicketSet?.status === 'pending'
        && Date.parse(state.pendingTicketSet.createdAt || 0) < startMs) {
      const stale = state.pendingTicketSet;
      state.pendingTicketSet = {
        ...stale,
        status: 'superseded',
        supersededAt: new Date().toISOString(),
      };
      logEvent('report_tickets_superseded', {
        ticket_set: stale.id,
        tickets: (stale.tickets || []).map((ticket) => ticket.number),
      });
      supersededTicketSet = stale;
    }
    if (brokerExecution && state.pendingBrokerExecution?.id === brokerExecution.id) {
  ...
  if (!ok) {
    ...
    const pendingForFailure = state.pendingTicketSet?.status === 'pending'
      ? state.pendingTicketSet : null;
    const failureOptions = pendingForFailure ? formatTicketOptions(pendingForFailure) : '';
  ...
  if (!brokerExecution
    ...
    if (itemsMessage) {
      const items = sanitizePhoneText(itemsMessage, {
        ...
        maxLines: ticketParsed.tickets.length + 5,
      });
  ...
  if (supersededTicketSet) {
    await sendText(
      replyTo,
      '♻️ Retired the prior pending ticket — this run reconciled the book and did not re-propose it, so no stale ticket carries forward.',
    );
  }
  if (!parsedReply.decision) {
...
function handleScheduledPost(req, res) {
  ...
  req.on('end', () => {
    ...
    state.scheduledRunAt = state.scheduledRunAt || {};
    if (isDuplicateScheduledRun(
      state.scheduledRunAt[kind],
      nowMs,
      { force: payload.force === true, kind },
    )) {
      logEvent('scheduled_double_fire_skipped', { kind, last: state.scheduledRunAt[kind] });
      res.writeHead(202);
      return res.end(`${kind} double-fire within debounce — skipped`);
    }
    state.scheduledRunAt[kind] = new Date(nowMs).toISOString();
    saveState(state);
