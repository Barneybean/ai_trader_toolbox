...
Usage:
  python3 scripts/quality_gate.py metrics/sym.json
  python3 scripts/quality_gate.py --metrics '{"ticker":"XYZ",...}' --exempt "net_margin=growth-stage: EBITDA turned positive FY25, scaling"
"""
import argparse
import json
import sys
...
def main():
    ...
    for key, bad, desc in FILTERS:
        v = m.get(key, None)
        if v is None:
            if key == "interest_coverage" and m.get("no_debt"):
