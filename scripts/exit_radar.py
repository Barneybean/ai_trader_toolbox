...
pre-committed level, not a mood.
    ...
    python3 scripts/exit_radar.py NVDA                     # Yahoo 2y daily
    python3 scripts/exit_radar.py MU --entry 150           # + give-back guard
    python3 scripts/exit_radar.py bars.json --entry 42.2   # connector bars file
    python3 scripts/exit_radar.py NVDA --float 2.3e10 --json
...
def analyze(bars, entry=None, float_shares=None):
    ...
    if len(c) >= 45:
        ...
        obv_hi_recent = max(obv[-5:]) >= max(obv[-40:]) * 0.999 if max(obv[-40:]) != 0 else True
    ...
    if r5:
        ...
        if (_percentile_rank(sorted(r5), recent_max) or 0) >= 95 and max(v[-10:]) >= 1.8 * vol20:
    ...
    try:
        centers, chips, _w = indicators.chip_bins(h, l, c, v, float_shares=float_shares)
        total = sum(chips)
