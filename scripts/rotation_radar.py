...
Codifies the highest-value pro behavior: de-risking a hot complex BEFORE it cracks and
rotating into neglected defensives while they are still unloved — not after the move.
The tell is almost always RELATIVE: a sector still "up big"
...
relative strength turns positive off a base.
    ...
    python3 scripts/rotation_radar.py            # sector ETF sweep vs SPY
    python3 scripts/rotation_radar.py QQQ SMH …  # custom universe

Per sector ETF (2y daily, via scripts/yahoo.py):
...
def ret(px, n):
    return 100.0 * (px[-1] / px[-1 - n] - 1) if len(px) > n else 0.0
...
def main():
    ...
    for sym in universe:
        try:
            ...
            continue
        rs63, rs21 = ret(px, 63) - s63, ret(px, 21) - s21
    ...
    print("\nCOOLING = the mentor signal (crack before the headline) -> trim leaders TO A BASE."
          "\nTURNING/NEGLECTED = rotation destinations -> underwrite, buy weakness, never chase.")
