...
invention. Price/bars auto-pull from Yahoo (scripts/yahoo.py) when omitted.
...
or price reclaiming SMA50) -> BUY-CANDIDATE; washed but still knifing -> WAIT.
    ...
    python3 scripts/value_radar.py --metrics candidates.json [--json]
...
def _bars_metrics(ticker):
    ...
    obv_slope60 = obv_series[-1] - obv_series[-60]
...
def evaluate(m):
    ...
    if all(k in m for k in ("fcf", "shares", "growth")):
    ...
    if all(k in m for k in ("eps_bear", "pe_bear")):
    ...
    if band in ("VALUE_30", "DEEP_VALUE"):
        if tech.get("washed") and tech.get("basing") and bear_ok:
...
def main():
    ...
    rows = [evaluate(m) for m in json.load(open(args.metrics))]
