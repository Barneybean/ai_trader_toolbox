...
Usage:
  python3 scripts/analysis/forecast.py MP.json --price 53.48 --horizon 20 --sims 20000
  python3 scripts/analysis/forecast.py MP.json --price 53.48 --drift -0.15        # -15%/yr view
  python3 scripts/analysis/forecast.py MP.json --peers NVDA.json,TSLA.json        # add analog pool
  python3 scripts/analysis/forecast.py MP.json --json                             # machine-readable
"""
...
def load_bars(source, rng="5y"):
    ...
    Finance (see scripts/lib/yahoo.py). Yahoo gives far more history than the Robinhood
    ...
    raw = json.loads(raw)
    bars = ind._find_bars(raw)
    if not bars:
        raise SystemExit(json.dumps({"error": f"no bars found in {source}"}))
    return ind._ohlcv(bars)
...
def find_tested_support(lows, closes, price, lookback=45, tol_pct=2.0):
    ...
    base = min(win_lo)
    if base <= 0:
        return None, 0, None   # corrupt tick (zero/negative low) — no shelf claim
...
def historical_analogs(series_list, horizon=20, squeeze_tercile=0.33,
    ...
    for (h, l, c) in series_list:
        ...
        for i in range(40, len(c) - horizon):
            ...
            if base <= 0 or (c[i] - base) / base * 100 > support_tol:
...
def block_bootstrap_paths(rets, price, horizon, sims, block=5, drift_daily=0.0, rng=None):
    ...
    n = len(rets)
    if n == 0:
        raise SystemExit(json.dumps({"error": "no usable returns — need >=2 bars with positive closes"}))
    for _ in range(sims):
        steps = []
        while len(steps) < horizon:
            # n - block + 1 so the freshest return is reachable (start can point
            # at the last full block; randrange's stop is exclusive)
...
def summarize_paths(paths, price, up_trigger, dn_trigger):
    ...
    for p in paths:
        tu = td = None
        # skip step 0 (the starting price): a trigger at/inside the current
        # price must be *reached*, not counted as touched before the path moves
...
def build(o, h, l, c, v, price, horizon, sims, drift_annual, up_trigger, dn_trigger,
    ...
    # just under the tested shelf; if no valid shelf, a plain -5% line
    dn_trigger = dn_trigger or (round(sup * 0.985, 2) if sup else round(price * 0.95, 2))
    ...
    return {
        ...
        "squeeze": (lambda wp: {
            ...
            "width_percentile": wp,
            "is_squeeze": (wp or 100) <= 33,
        })(percentile_rank(widths, cur_w)),
...
def _fmt(r):
    ...
    if s["level"] is None:
        L.append("  Support tested  (no valid shelf — corrupt/zero low in window)")
    else:
...
def main(argv=None):
    ...
    a = p.parse_args(argv)
    if a.drift <= -0.99:
        p.error("--drift must be > -0.99 (annualized fraction, e.g. -0.15)")

    o, h, l, c, v = load_bars(a.source, a.range)
    if len(c) < 15:
        print(json.dumps({"error": f"insufficient data: {len(c)} bars (need >=15)"}))
        return 1
    price = a.price if a.price is not None else c[-1]
    peers = []
    if a.peers:
