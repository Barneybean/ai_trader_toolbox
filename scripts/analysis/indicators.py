...
def obv_series(closes, volumes):
    """Full On-Balance-Volume series (seeded at 0). Shared by obv(), the flow
    engine's divergence read, and the value/exit radars."""
    o = [0.0]
    for i in range(1, len(closes)):
...
def volume_read(volumes, period=20):
    ...
    return {
        "last": round(last, 0),
        "avg_20": round(avg, 0),
        "rel_volume": round(ratio, 2) if ratio is not None else None,
        "state": ("expanding" if ratio is not None and ratio > 1.5
                  else "contracting" if ratio is not None and ratio < 0.6
                  else "normal"),
    }
...
def chip_bins(highs, lows, closes, volumes, float_shares=None, bins=48, half_life=55):
    ...
    for i in range(len(closes)):
        ...
        for b in range(bins):
            ...
            eb_hi = eb_lo + width
            if span <= 0:
                # top bin is inclusive so a flat bar at the global max isn't dropped
                if eb_lo <= lo < eb_hi or (b == bins - 1 and lo >= eb_hi):
                    chips[b] += vol
                    break
                continue
...
def chip_distribution(highs, lows, closes, volumes, price, float_shares=None,
    ...
    conc_pct = round(conc_width / price * 100, 1) if conc_width is not None else None
    conc_label = None
...
def build(bars, price_override=None, float_shares=None):
    ...
    result = {
        ...
        "range_52w": {"high": round(max(h[-252:] if has_hl else c[-252:]), 4),
                      "low": round(min(l[-252:] if has_hl else c[-252:]), 4),
                      "bars_covered": min(len(c), 252)},
        "trade_scaffold": plan,
    }
