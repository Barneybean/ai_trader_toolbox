...
(a list of {begins_at, open, high, low, close, volume} dicts).
    ...
    python3 scripts/lib/yahoo.py MP                         # 5y daily -> stdout
    python3 scripts/lib/yahoo.py MP --range 10y --out reports/cache/market-data/MP.json
    python3 scripts/lib/yahoo.py MP --raw                   # unadjusted (actual traded prices)

Then feed the file to any of the analytics:

    python3 scripts/analysis/forecast.py reports/cache/market-data/MP.json --price 53.31
    python3 scripts/analysis/indicators.py reports/cache/market-data/MP.json --price 53.31
...
def fetch(symbol, rng="5y", interval="1d", adjusted=True, timeout=20):
    ...
    if not symbol or len(symbol) > 30 or any(ch.isspace() for ch in symbol):
        raise ValueError("symbol must be a non-empty Yahoo ticker without whitespace")
    ...
    for i, t in enumerate(ts):
        ...
        if not isinstance(close, (int, float)) or not math.isfinite(close) or close <= 0:
            # Yahoo pads holidays/halts with nulls; reject malformed prices too.
            continue
        # Back-adjustment factor (split + dividend). ~1.0 for recent bars.
        adj_close = adj[i] if adjusted and adj and i < len(adj) else None
        f = (adj_close / close) if (isinstance(adj_close, (int, float))
                                    and math.isfinite(adj_close) and adj_close > 0) else 1.0
        ...
        vv = vol[i] if i < len(vol) and isinstance(vol[i], (int, float)) \
            and math.isfinite(vol[i]) and vol[i] >= 0 else 0
