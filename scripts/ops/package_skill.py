...
single source of truth on demand. Never hand-edit the zip; re-run this after any change.
    ...
    python3 scripts/ops/package_skill.py            # -> dist/ai-trader.zip
    python3 scripts/ops/package_skill.py --out /tmp # custom output dir
...
INCLUDE_TOP = ["SKILL.md", "AGENTS.md", "PORTABILITY.md", "README.md", "LICENSE",
               "skills", "scripts"]
# Never bundle these, at any depth. "*.local.*" is load-bearing: the git-ignored
# denylists (scripts/ops/*_denylist.local.txt) hold real account numbers and
# personal markers — they must never leave the machine inside the zip.
EXCLUDE_GLOBS = ["*.pyc", "__pycache__", ".DS_Store", "*.zip", "*.local.*",
                 "*.env", ".env*", "logs"]
