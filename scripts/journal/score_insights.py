...
Registry: journal/insights.jsonl — one JSON object per line:
  {"id": "sym-long-20260705", "ts": "2026-07-05", "ticker": "SYM",
   "direction": "up",              # up | down | flat
   "magnitude": 0.70,              # expected move in direction, decimal (0.70 = +70%)
   "confidence": 0.60,             # 0-1
   "price_at_call": 42.74,
   "expires": "2027-07-05",
   "methods": ["layer-rotation", "chip-distribution"],   # generating method tags
   "thesis": "sym-robotics-definer",                     # GroupId linking horizons
   "source": "report_2026-07-05_ai-robotics-value-chain",
...
Usage:
  python3 scripts/journal/score_insights.py          # score expired insights
  python3 scripts/journal/score_insights.py --mark   # also mark-to-market open ones
  python3 scripts/journal/score_insights.py --json
"""
import argparse
import json
import os
...
def direction_hit(direction, ret):
    ...
    if direction == "flat":
        return abs(ret) <= 0.05
    raise ValueError(f"bad direction {direction!r}; expected up, down, or flat")
...
def main():
    ...
    if not recs:
        ...
        print("[]" if args.json else f"no insights registered ({args.registry})")
        return 0
    ...
    for rec in recs:
        try:
            ...
            if not expired and not args.mark:
                ...
                continue
            signed_exp = ({"up": 1, "down": -1, "flat": 0}.get(rec.get("direction"), 0)
                          * float(rec.get("magnitude", 0)))
            rows.append({
                **rec,
                ...
                "hit": direction_hit(rec.get("direction"), ret),
                "mag_error": round(abs(ret - signed_exp), 4),
            })
        except Exception as e:  # one malformed row must not kill the whole run
            rows.append({**rec, "status": "ERROR", "detail": f"{type(e).__name__}: {e}"})
    ...
    for r in rows:
        if "realized" in r:
            print(f"{r['id']:<28} {r['status']:<15} {r['direction']:<5} {r.get('confidence') or 0:>4.2f} "
