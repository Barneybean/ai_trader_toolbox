...
def _load():
    _ensure()
    # strict: this file gets rewritten by _save_all — tolerant-skip here would
    # silently DELETE a malformed line on the next save
    from jsonl import load_jsonl
    return load_jsonl(DECISIONS, tolerant=False)


def _outcome(r):
    """The structured score dict, or {} — legacy rows carry free-text outcomes
    (fill notes) which must render as unscored, not crash."""
    o = r.get("outcome")
    return o if isinstance(o, dict) else {}


BEARISH_ACTIONS = ("trim", "avoid", "sell", "sell-all", "exit", "veto", "underweight")
...
def cmd_score(args):
    ...
    # a trim/avoid/sell call WORKS when the price falls — grade on the call's side
    bearish = str(rec.get("action", "")).lower() in BEARISH_ACTIONS
    call_ret = round(-raw, 2) if (bearish and raw is not None) else raw
    spy = _pct(args.spy_entry, args.spy_exit) if args.spy_entry and args.spy_exit else None
    # An avoided/short loss is measured against owning SPY: SPY return - asset
    # return.  Subtracting SPY from the inverted asset return double-counts the
    # market direction and penalizes a correct defensive call in a rising tape.
    alpha = (round(call_ret + spy, 2) if bearish else round(call_ret - spy, 2)) \
        if (call_ret is not None and spy is not None) else None
    # verdict vs the plan: hit target / stopped / in-between, on the call's own side
    entry, stop, target = rec.get("entry"), rec.get("stop"), rec.get("target")
    verdict = "open"
    if not bearish:
        if target and args.exit >= target:
            verdict = "target_hit"
        elif stop and args.exit <= stop:
            verdict = "stopped_out"
    else:
        # bearish target below entry = fall-to level; above entry = sell-into-strength.
        # bearish stop is the invalidation ABOVE entry (price rallying through it).
        if target and entry and ((target < entry and args.exit <= target)
                                 or (target > entry and args.exit >= target)):
            verdict = "target_hit"
        elif stop and entry and stop > entry and args.exit >= stop:
            verdict = "stopped_out"
    if verdict == "open" and call_ret is not None:
        verdict = "win" if call_ret > 0 else "loss"
    if alpha is not None:
        verdict += "_beat_spy" if alpha > 0 else "_lag_spy"
    rec["outcome"] = {"exit": args.exit, "raw_return": raw, "call_return": call_ret,
                      "side": "bearish" if bearish else "long", "spy_return": spy,
...
def cmd_report(args):
    # outcome must be the structured score dict; free-text outcomes (fill notes) aren't scores
    rows = [r for r in _load() if isinstance(r.get("outcome"), dict)]
    if args.symbol:
        rows = [r for r in rows if r["symbol"] == args.symbol.upper()]
    if not rows:
        ...
        return
    n = len(rows)
    # call_return is side-adjusted (bearish inverted); older rows only have raw
    wins = sum(1 for r in rows
               if (r["outcome"].get("call_return")
                   if r["outcome"].get("call_return") is not None
                   else r["outcome"].get("raw_return") or 0) > 0)
