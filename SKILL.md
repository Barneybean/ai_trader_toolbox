...
- *Fundamental* — valuation, health, quality, **strategy & roadmap** (forward story + rate-sensitivity). Route by industry to `skills/analysis/sector-playbooks.md` for the KPIs/catalysts that set the price; if `skills/analysis/stocks/<TICKER>.md` exists, read it too. Primary sources: transcript, filings, **backlog/RPO/capacity** vs. what estimates embed, value-chain read. Before an actionable single-name call, apply `skills/analysis/valuation-quality-gate.md`. Every single-name deep dive also runs `skills/analysis/business-inflection.md`: inventory material strategic changes, map estimates versus multiple channels, decide what is priced, and provide the dated inflection table. Build scenario valuation (bull/base/bear × probability) on binaries. (Core sleeve.)
...
1. `python3 scripts/report/new_report.py --market <open|closed> --title <short-run-title> --model <your-model-name>` → prints the `.md` path in `reports/.build/` (write the final content in). Title = the run's focus (e.g. `daily-desk-run`, `ai-robotics-value-chain`); model = the AI model running the desk (e.g. `claude-fable-5`).
2. Charts per covered name: `python3 scripts/report/charts.py <historicals.json> --symbol <T> --price <live> --float <float> --action <..> --conviction <..> --score <N> --sleeve <..> --entry/--stop/--target <px> --date <YYYY-MM-DD>` — chart sources auto-file under `reports/assets/charts/YYYY-Www/`; embed the **scorecard** (lead visual) + price/**forecast**/chips/gauges it prints.
