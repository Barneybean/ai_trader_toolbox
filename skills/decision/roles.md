...
## Data / Ops
**Mandate:** Pull clean data from the Robinhood connector per candidate (quote, fundamentals, historicals, earnings, tradability, position, buying power). Run `scripts/analysis/indicators.py`. Hand one tidy fact-sheet per ticker to analysts. If a pull fails, say so — downstream roles must know what's missing.
