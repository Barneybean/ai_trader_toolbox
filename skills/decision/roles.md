...
**Output:** regime + setup classification (trend-continuation, breakout, pullback, mean-reversion, none), support/resistance map (nearest support, nearest resistance/pressure, ladders for scale-outs), momentum/volume confirmation, **phase + chip-distribution read + wash-vs-markup call**, RR estimate with EV check, and a timing verdict by horizon: **short-run (days–weeks)** entry/level + wash likelihood, and how it feeds the **long-run (6–18 months)** target with a time window.
