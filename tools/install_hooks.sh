...
chmod +x .githooks/pre-commit .githooks/pre-push tools/scan_pii.py 2>/dev/null || true
...
echo
echo "Tip: add your exact private strings (account #s, names) to tools/pii_denylist.local.txt"
echo "     (git-ignored) so the gate hard-blocks them too. See tools/pii_denylist.example.txt."
